...
import SourcesSchemesManager from '../../../components/Admin/SourcesSchemesManager';
...
export default function AdminBusinessForm() {
  ...
  useEffect(() => {
    api.adminGetCategories().then(setCategories).catch(() => setCategories([]));
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const biz = await api.adminGetBusiness(id);
        if (cancelled) return;
        const gc = safeParse(biz.growth_chart, {});
        const ic = safeParse(biz.investment_chart, {});
        const pp = safeParse(biz.profit_projection, []);
        setForm({
          name: biz.name || '', category_id: biz.category_id || '', market_size: biz.market_size || '',
          growth_rate: biz.growth_rate || '', investment: biz.investment || '',
          min_investment: biz.min_investment || '', max_investment: biz.max_investment || '',
          profit_margin: biz.profit_margin || '',
          breakeven: biz.breakeven || '', breakeven_value: biz.breakeven_value || '', breakeven_unit: biz.breakeven_unit || 'Months',
          overview: biz.overview || '', is_hidden: !!biz.is_hidden,
          badges: safeParse(biz.badges, []),
          roadmap: safeParse(biz.roadmap, []),
          suppliers: safeParse(biz.suppliers, []),
          competitors: safeParse(biz.competitors, []),
        ...
        toast.error('Could not load this business.');
      } finally {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (isEdit) {
        await api.adminUpdateBusiness(id, payload);
        toast.success('Business updated.');
      } else {
        const res = await api.adminAddBusiness(payload);
        toast.success('Business created.');
        navigate(`/admin/businesses/${res.id}/edit`, { replace: true });
        return;
      }
    } catch (err) {
      toast.error(err?.response?.data?.error || 'Could not save business.');
    } finally {
      setSaving(false);
    }
  }

  if (loading) return <div className="admin-empty">Loading business…</div>;

  return (
    <div>
      <div className="admin-topbar">
        <div>
          <div className="admin-page-title">{isEdit ? `Edit: ${form.name}` : 'Add Business'}</div>
          <div className="admin-page-sub">All fields feed the public Business Details page directly.</div>
        </div>
        <Button variant="ghost" icon="ti-arrow-left" onClick={() => navigate('/admin/businesses')}>Back to list</Button>
      </div>
      ...
      <form onSubmit={submit}>
        <FormSection title="Basic Info" icon="ti-info-circle" defaultOpen>
          <div className="admin-field">
            <label>Name *</label>
            <input type="text" value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Cloud Kitchen" />
          </div>
          <div className="admin-field">
        ...
        <FormSection title="Financials" icon="ti-report-money" defaultOpen>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="admin-field">
              ...
              <input type="text" inputMode="numeric" value={form.market_size} onChange={(e) => set('market_size', e.target.value.replace(/[^0-9.]/g, ''))} placeholder="50000000" />
            </div>
            <div className="admin-field">
              <label>Growth Rate</label>
              <input type="text" value={form.growth_rate} onChange={(e) => set('growth_rate', e.target.value)} placeholder="18%" />
            </div>
            <div className="admin-field">
              <label>Investment (display text)</label>
              <input type="text" value={form.investment} onChange={(e) => set('investment', e.target.value)} placeholder="₹2L – ₹5L" />
            </div>
            <div className="admin-field">
              <label>Profit Margin</label>
              <input type="text" value={form.profit_margin} onChange={(e) => set('profit_margin', e.target.value)} placeholder="25%" />
            </div>
            <div className="admin-field">
            ...
            <div className="admin-field">
              ...
              <input type="text" inputMode="numeric" value={form.breakeven_value} onChange={(e) => set('breakeven_value', e.target.value.replace(/[^0-9.]/g, ''))} placeholder="8" />
            </div>
            <div className="admin-field">
        ...
        <FormSection title="Overview" icon="ti-file-description">
          <textarea rows={4} value={form.overview} onChange={(e) => set('overview', e.target.value)} placeholder="Business description shown on the details page…" style={{ width: '100%' }} />
        </FormSection>
        ...
        <FormSection title="Highlight Badges" icon="ti-star">
          <RepeatableRows
            ...
            fields={[{ key: 'label', placeholder: 'e.g. High Growth' }]}
            emptyRow={{ label: '' }}
            addLabel="Add badge"
          />
        </FormSection>
        ...
        <FormSection title="Investment Breakdown (% of capital)" icon="ti-chart-donut">
          <RepeatableRows
            ...
            fields={[
              { key: 'label', placeholder: 'e.g. Equipment', width: 2 },
        ...
        <FormSection title="Profit Projection (per year)" icon="ti-chart-bar">
          <RepeatableRows
            ...
            fields={[{ key: 'value', placeholder: 'e.g. 4.5 (₹ Lakh)', type: 'number' }]}
            emptyRow={{ value: '' }}
            addLabel="Add year"
          />
        </FormSection>

        <FormSection title="Business Roadmap" icon="ti-route">
          <RepeatableRows
            items={form.roadmap}
        ...
        {isEdit && (
          <FormSection title="Sources & Government Schemes" icon="ti-shield-check">
            <SourcesSchemesManager businessId={id} />
          </FormSection>
        )}
